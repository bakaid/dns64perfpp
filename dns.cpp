...
bool DNSLabel::isPointer() const {
	return (begin_[0] & 0xc0) == 0xc0;
}
...
DNSQName::DNSQName(uint8_t* begin, size_t maxlen, DNSPacket& packet): begin_{begin},
																	  packet_{packet} {
	uint8_t* iter = begin;
...
DNSQuestion::DNSQuestion(uint8_t* begin, size_t maxlen, DNSPacket& packet): begin_{begin},
																			name_{DNSQName{begin, maxlen, packet}} {
	size_t len = maxlen;
...
void DNSQuestion::qtype(uint16_t q) {
	*qtype_ = htons(q);
}

uint16_t DNSQuestion::qclass() const {
	return ntohs(*qclass_);
}

void DNSQuestion::qclass(uint16_t q) {
	*qclass_ = htons(q);
}


DNSResource::DNSResource(uint8_t* begin, size_t maxlen, DNSPacket& packet): begin_{begin},
																						name_{DNSQName{begin, maxlen, packet}},
																						packet_{packet} {
	size_t len = maxlen;
	...
	//
	if (len < sizeof(uint32_t)) throw std::out_of_range{"Packet too small"};
	...
	//
	if (rdlength() > len) throw std::out_of_range{"Packet too small"};
	rdata_ = begin;
}

uint16_t DNSResource::qtype() const {
	return ntohs(*qtype_);
}

void DNSResource::qtype(uint16_t q) {
	*qtype_ = htons(q);
}

uint16_t DNSResource::qclass() const {
	return ntohs(*qclass_);
}

void DNSResource::qclass(uint16_t q) {
	*qclass_ = htons(q);
}

uint32_t DNSResource::ttl() const {
	return ntohl(*ttl_);
}

void DNSResource::ttl(uint32_t ttl) {
	*ttl_ = htonl(ttl);
}

uint16_t DNSResource::rdlength() const {
	return ntohs(*rdlength_);
}

void DNSResource::rdlength(uint16_t r) {
	*rdlength_ = htons(r);
}

uint8_t* DNSResource::rdata() const {
	return rdata_;
}
...
size_t DNSResource::rdataToString(char* buffer, size_t maxlen) const {
	...
	switch (qtype()) {
		...
		case QType::MX:
		{
			...
			DNSQName name{rdata() + sizeof(preference), rdlength() - sizeof(preference), packet_};
...
void DNSPacket::resize(uint8_t* begin, size_t oldsize, size_t newsize) {
	...
	for (auto& question: question_) {
		if (question.begin_ > begin) {
			...
			question.qtype_ = reinterpret_cast<uint16_t*>(reinterpret_cast<uint8_t*>(question.qtype_) + (newsize - oldsize));
			question.qclass_ = reinterpret_cast<uint16_t*>(reinterpret_cast<uint8_t*>(question.qclass_) + (newsize - oldsize));
		}
	}
	for (auto& resource: answer_) {
		if (resource.begin_ > begin) {
			resource.begin_ += (newsize - oldsize);
			resource.qtype_ = reinterpret_cast<uint16_t*>(reinterpret_cast<uint8_t*>(resource.qtype_) + (newsize - oldsize));
			resource.qclass_ = reinterpret_cast<uint16_t*>(reinterpret_cast<uint8_t*>(resource.qclass_) + (newsize - oldsize));
			resource.ttl_ = reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(resource.ttl_) + (newsize - oldsize));
			resource.rdlength_ = reinterpret_cast<uint16_t*>(reinterpret_cast<uint8_t*>(resource.rdlength_) + (newsize - oldsize));
	...
	memmove(begin + newsize, begin + oldsize, len_ - ((begin + oldsize) - begin_));
