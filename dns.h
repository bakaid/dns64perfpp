...
class DNSHeader
{
	...
	public:
		// getters
		inline uint16_t id() const {return ntohs(id_);} /**< Getter for the DNS Query identifier. */
		inline bool qr() const {return (ntohs(flags_) >> 15) & 0x1;} /**< Getter for Query(0)/Response(1) flag. */
		inline uint8_t opcode() const {return (ntohs(flags_) >> 11) & 0xf;} /**< Getter for Opcode. */
		inline bool aa() const {return (ntohs(flags_) >> 10) & 0x1;} /**< Getter for the Authoritative Answer flag. */
		inline bool tc() const {return (ntohs(flags_) >> 9) & 0x1;} /**< Getter for the Truncation flag. */
		inline bool rd() const {return (ntohs(flags_) >> 8) & 0x1;} /**< Getter for the Recursion Desired flag. */
		inline bool ra() const {return (ntohs(flags_) >> 7) & 0x1;} /**< Getter for the Recursion Avaliable flag. */
		inline uint8_t rcode() const {return ntohs(flags_) & 0xf;} /**< Getter for the Response Code. */
		inline uint16_t qdcount() const {return ntohs(qdcount_);} /**< Getter for the Question Count. */
		inline uint16_t ancount() const {return ntohs(ancount_);} /**< Getter for the Answer Resource Count. */
		inline uint16_t nscount() const {return ntohs(nscount_);} /**< Getter for the Authority Resource Count. */
		...
		inline void arcount(uint16_t arcount_s) {arcount_ = htons(arcount_s);} /**< Setter for the Additional Resource Count. */
...
struct DNSLabel
{
	...
	/**
	 * Returns the actual size of the label in the packet (2 for pointers, length() + 1 for normal labels)
	 * @return the size
	 */
	size_t size() const;
	
	/**
	 * Returns the length of the label (0 for pointers, the number of characters in the label for normal labels)
	 * @return the length
	 */
	size_t length() const;
};
...
struct DNSQName
{
	 ...
	 */
	size_t labelsToString(std::vector<DNSLabel>::iterator first, char* buffer, size_t maxlen) const;
...
struct DNSResource
{
	 ...
	 * Warning: DO NOT use this alone. The rdata setter automatically changes this field too.
...
struct DNSPacket
{
	...
	std::vector<DNSQuestion> question_; /**< All the Questions contained in the packet. */
	std::vector<DNSResource> answer_; /**< All the Answer Resources contained in the packet. */
	std::vector<DNSResource> authority_; /**< All the Authority Resources contained in the packet. */
	std::vector<DNSResource> additional_; /**< All the Additional Resources contained in the packet. */
