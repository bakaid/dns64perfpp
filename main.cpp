...
int main(int argc, char* argv[]) {
	struct in6_addr source_addr, server_addr;
	uint16_t port;
	uint32_t ip;
	uint8_t netmask;
	...
	if (argc < 8) {
		std::cerr << "Usage: dns64perf++ <source> <server> <port> <subnet> <number of requests> <burst size> <delay between bursts in ns>" << std::endl;
		return -1;
	}
	/* Source address */
	if (inet_pton(AF_INET6, argv[1], reinterpret_cast<void*>(&source_addr)) != 1) {
		std::cerr << "Bad source adddress." << std::endl;
		return -1;
	}
	/* Server address */
	if (inet_pton(AF_INET6, argv[2], reinterpret_cast<void*>(&server_addr)) != 1) {
	...
	/* Subnet */
	uint8_t temp[4];
	if (sscanf(argv[4], "%hhu.%hhu.%hhu.%hhu/%hhu", temp, temp+1, temp+2, temp+3, &netmask) != 5) {
	...
	/* Burst size */
	if (sscanf(argv[6], "%u", &num_burst) != 1) {
	...
	try {
		DnsTester tester{source_addr, server_addr, port, ip, netmask, num_req, num_burst, std::chrono::nanoseconds{burst_delay}};
		tester.start();
		tester.display();
